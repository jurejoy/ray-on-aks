# Configurations, Ray cluster on kubernetes(AKS)
# 1 ray head node without GPU, 2 ray worker nodes with 1 GPU each
# This script deploys a model with tensor parallelism across 2 GPUs on 2 worker nodes
import os
import ray
...
# Define deployment for distributed GPUs
@serve.deployment(
...
class LlamaTextGenerator:
    def __init__(self):
        ...
        self.llm = None
        logger.info(f"Initializing LlamaTextGenerator")
        ...
        # Load model with tensor parallelism - adjust parameters for better memory management
        logger.info(f"Loading model {self.model_id} with tensor parallelism...")
        try:
            self.llm = LLM(
                model=self.model_id,
                ...
                download_dir=None, 
                dtype="float16",
                gpu_memory_utilization=0.7,  # Lower utilization to avoid OOM
                max_num_seqs=32,  # Limit batch size
                max_num_batched_tokens=4096,  # Limit tokens per batch
                tensor_parallel_size=2  # Split across 2 GPUs
            )
            logger.info("Model loaded successfully with tensor parallelism across 2 GPUs")
