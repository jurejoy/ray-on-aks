...
from peft import LoraConfig, get_peft_model, PeftModel, PeftConfig, __version__ as peft_version
import ray
...
def training_function(kwargs: dict):
    ...
    # Set use_cache in the config instead of as a constructor parameter
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        ignore_mismatched_sizes=True,  # Add this parameter to handle vocab size differences
    )
    ...
    print(f"Model vocab size: {model.config.vocab_size}, Tokenizer vocab size: {len(tokenizer)}")
    ...
    # This needs to be updated inside the training_function
    # After model creation but before training
    # When resuming from a LoRA checkpoint, we need a different approach
    if "resume_from_checkpoint" in config and config["resume_from_checkpoint"]:
        ...
        # Extract vocab size information from the checkpoint
        ...
        if os.path.exists(tokenizer_config_path):
            with open(tokenizer_config_path, 'r') as f:
                ...
                if 'vocab_size' in tokenizer_config:
                    checkpoint_vocab_size = tokenizer_config['vocab_size']
        ...
        if os.path.exists(adapter_model_path) and os.path.exists(adapter_config_path):
            print(f"Detected LoRA checkpoint at {checkpoint_path}")
            
            # Skip loading adapter in distributed training
            print("WARNING: LoRA checkpoint loading with DeepSpeed is problematic")
            print("Starting with fresh model + LoRA configuration from checkpoint")
            
            try:
                # Create the PEFT configuration from the checkpoint
                fresh_peft_config = PeftConfig.from_pretrained(checkpoint_path)
                print(f"Using LoRA config from checkpoint: {fresh_peft_config}")
                
                # Apply LoRA to model based on checkpoint config
                print("Applying fresh LoRA configuration to base model")
                model = get_peft_model(model, fresh_peft_config)
                print("Successfully created LoRA model with checkpoint config")
                
                # Add these lines to fix parameter visibility with DeepSpeed
                trainable_params = []
                trainable_param_ids = set()  # Track parameters by ID to avoid duplicates

                # First collect parameters that are already trainable
                for param_name, param in model.named_parameters():
                    if param.requires_grad:
                        param_id = id(param)
                        if param_id not in trainable_param_ids:
                            trainable_params.append(param)
                            trainable_param_ids.add(param_id)
                            
                print(f"Found {len(trainable_params)} unique trainable parameters")

                if len(trainable_params) == 0:
                    print("WARNING: No trainable parameters found! Forcing LoRA layers to be trainable")
                    # Force LoRA parameters to be trainable
                        ...
                        if 'lora_' in name:
                            for param_name, param in module.named_parameters():
                                ...
                                if param_id not in trainable_param_ids:
                                    trainable_params.append(param)
                                    trainable_param_ids.add(param_id)
                    print(f"After forcing: Found {len(trainable_params)} unique trainable parameters")

                # Skip loading adapter weights due to DeepSpeed incompatibility
                print("WARNING: Skipping adapter weights loading due to DeepSpeed compatibility issues")
                print("Training will start from scratch with the same LoRA architecture")
                
            except Exception as e:
                print(f"Error setting up LoRA model: {str(e)}")
                import traceback
                traceback.print_exc()
                if not config["lora"]:
                    print("ERROR: Trying to use a LoRA checkpoint but --lora flag not set!")
                    raise RuntimeError("Please restart with --lora flag to use this checkpoint")
        else:
            # Handle non-LoRA checkpoints here
            print(f"No LoRA adapter model found at {checkpoint_path}")
            print("Looking for regular pytorch_model.bin file...")
            
            # Try regular model loading
            ...
            if os.path.exists(pytorch_model_path):
                ...
                print("Successfully loaded regular model weights")
            else:
                print(f"No model weights found at {checkpoint_path}")
                print("Continuing without loading checkpoint weights")
    ...
    optimizer = optimizer_cls(
        trainable_params,  # Use explicit list instead of model.parameters()
        lr=lr,
        betas=OPTIM_BETAS,
    ...
    # THEN load optimizer state if it exists
    if "resume_from_checkpoint" in config and config["resume_from_checkpoint"]:
        checkpoint_path = config["resume_from_checkpoint"]
        optimizer_file = os.path.join(checkpoint_path, "optimizer.pt")
        if os.path.exists(optimizer_file):
...
def main():
    ...
    # Update config to include resume_from_checkpoint
    if args.resume_from_checkpoint:
        config["resume_from_checkpoint"] = args.resume_from_checkpoint

    ray.init(
        runtime_env={
            "env_vars": {"HF_HOME": "/shared/.cache/huggingface"},
            "working_dir": ".",
        }
    )
    ...
    # Create a Checkpoint object if resuming from checkpoint
    checkpoint_to_resume = None
    if args.resume_from_checkpoint:
        try:
            checkpoint_to_resume = train.Checkpoint.from_directory(args.resume_from_checkpoint)
            print(f"Created Checkpoint object from: {args.resume_from_checkpoint}")
        except Exception as e:
            print(f"Error creating checkpoint object: {e}")
    ...
    trainer = TorchTrainer(
        ...
        # Use the Checkpoint object instead of the string path
